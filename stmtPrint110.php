    ...
    <?php
				foreach ( $CONTACTS as $k => $v ) {
					...
					while ( $getPR = $getPay->fetch () ) {
						$sb += ($getPR ['debitAmount'] - $getPR ['creditAmount']);
					}
					if ($sb != 0.00) {
						?>
...
<tr>
<td colspan="5" style="text-align:left; font-weight:bold; padding:30px 0px;">
...
<?php
						$getX = $db->prepare ( "SELECT * FROM $myFLedger WHERE contact = ? AND accountNumber = ? ORDER BY date" );
						$getX->execute ( array (
								$k,
								'110.0'
						) );
						while ( $getXR = $getX->fetch () ) {
							...
							if ($debitAmount >= .01 || $creditAmount >= .01) {
								?>
	<tr>
<td style="text-align:left; font-weight:bold; border-left:2px solid #000000; border-right:1px solid #000000;">
<?php
