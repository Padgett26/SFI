<div id='mainTableBox' style="padding:10px;">
    ...
    if (filter_input(INPUT_GET, 'ver', FILTER_SANITIZE_STRING)) {
        ...
        if ($row) {
            ...
            if ($ver == $link) {
                $stmt2 = $db->prepare(
                        "UPDATE users SET verifyCode=?, accessLevel = ? WHERE id=?");
                $stmt2->execute(array(
                        '0',
                        "1",
                        $rId
    ...
    if (filter_input(INPUT_POST, 'myInfoUp', FILTER_SANITIZE_STRING) == "new") {
        ...
        if (filter_input(INPUT_POST, 'email', FILTER_VALIDATE_EMAIL)) {
            ...
            $stmt = $db->prepare(
                    "SELECT COUNT(*) FROM users WHERE email=? && accessLevel >= ?");
            $stmt->execute(array(
                    $newEmail,
                    "1"
            ));
            ...
            if ($email == '0') {
                ...
                if ($pwd1 != "" && $pwd1 != " " && $pwd1 === $pwd2) {
                    ...
                    $stmt2 = $db->prepare(
                            "SELECT id FROM users WHERE email=? && password=? ORDER BY id DESC LIMIT 1");
                    $stmt2->execute(array(
                            $email,
                            $hidepwd
                    ));
