	...
	if (filter_input ( INPUT_POST, 'closeDay', FILTER_SANITIZE_NUMBER_INT ) == 1) {
		...
		foreach ( $_POST as $key => $val ) {
			...
			if (preg_match ( "/^amt([0-9][0-9]*)$/", $key, $match )) {
				...
				if ($v >= 0.01) {
					$n = getNext ( '5', $db, $myFLedger );
					$ID = $match [1];
					$getAccount = $db->prepare ( "SELECT accountNumber FROM $myFAccounts WHERE id = ?" );
					...
					$upLedger1 = $db->prepare ( "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,'0.00',?,?,?,'1','0','0')" );
					...
					$upLedger2 = $db->prepare ( "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,?,'0.00',?,?,'1','0','0')" );
		...
		foreach ( $confirm as $value ) {
			$setConfirm = $db->prepare ( "UPDATE $myFLedger SET dailyConfirm = '1' WHERE id = ?" );
...
<div
	...
	foreach ( $CONTACTS as $k => $v ) {
		$sb = 0;
		$getS = $db->prepare ( "SELECT startingBalance FROM $myContacts WHERE id = ?" );
		$getS->execute ( array (
				$k
		) );
		$getSR = $getS->fetch ();
		...
		$getPay1 = $db->prepare ( "SELECT debitAmount, creditAmount FROM $myFLedger WHERE contact = ? AND accountNumber = ?" );
		...
		$getPay2 = $db->prepare ( "SELECT debitAmount, creditAmount FROM $myFLedger WHERE contact = ? AND accountNumber = ?" );
	...
	foreach ( $CONTACTS as $k => $v ) {
		$s = 0;
		$getsb = $db->prepare ( "SELECT startingBalance FROM $myContacts WHERE id = ?" );
		$getsb->execute ( array (
				$k
		) );
		$getsbR = $getsb->fetch ();
