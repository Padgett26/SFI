...
if ($myId >= 1 && $SA == 0) {
	...
	$an = (filter_input ( INPUT_GET, 'an', FILTER_SANITIZE_NUMBER_INT )) ? filter_input ( INPUT_GET, 'an', FILTER_SANITIZE_NUMBER_INT ) : '0'; // grab account
	$je = (filter_input ( INPUT_GET, 'je', FILTER_SANITIZE_NUMBER_INT )) ? filter_input ( INPUT_GET, 'je', FILTER_SANITIZE_NUMBER_INT ) : '0'; // grab journal entry
	$type = (filter_input ( INPUT_GET, 'type', FILTER_SANITIZE_NUMBER_INT )) ? filter_input ( INPUT_GET, 'type', FILTER_SANITIZE_NUMBER_INT ) : '0'; // grab typeCode
	...
	if (filter_input ( INPUT_POST, 'editA', FILTER_SANITIZE_STRING )) {
		...
		$upAccountType = filter_input ( INPUT_POST, 'accountType', FILTER_SANITIZE_NUMBER_INT );
		$upAccountNumber = filter_input ( INPUT_POST, 'accountNumber', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		$upAccountName = filter_var ( htmlEntities ( trim ( $_POST ['accountName'] ), ENT_QUOTES ), FILTER_SANITIZE_STRING );
		$upStartBalance = filter_input ( INPUT_POST, 'startBalance', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		$upTime = date2mktime ( filter_input ( INPUT_POST, 'startDate', FILTER_SANITIZE_NUMBER_INT ), 'start' );

		if ($aid == "new" && $upAccountType != "0") {
			$newA = $db->prepare ( "INSERT INTO $myFAccounts VALUES(NULL,?,?,?,?,?)" );
			$newA->execute ( array (
					$upAccountNumber,
			...
			$upA = $db->prepare ( "UPDATE $myFAccounts SET startBalance = ? WHERE id = ?" );
			$upA->execute ( array (
					$upStartBalance,
					$a
			) );
	...
	if (filter_input ( INPUT_POST, 'jUp', FILTER_SANITIZE_NUMBER_INT )) {
		...
		$jDate = date2mktime ( filter_input ( INPUT_POST, 'jDate', FILTER_SANITIZE_NUMBER_INT ), 'noon' );
		$_SESSION ['jDate'] = $jDate;
		$jContactName = htmlEntities ( trim ( filter_input ( INPUT_POST, 'jContactName', FILTER_SANITIZE_STRING ) ) );
		$jContactNameSelect = filter_input ( INPUT_POST, 'jContactNameSelect', FILTER_SANITIZE_NUMBER_INT );
		$jDescription = filter_var ( htmlEntities ( trim ( $_POST ['jDescription'] ), ENT_QUOTES ), FILTER_SANITIZE_STRING );
		...
		$jAccNumber = filter_input ( INPUT_POST, 'jAccNumber', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		$jTypeCode = filter_input ( INPUT_POST, 'jTypeCode', FILTER_SANITIZE_NUMBER_INT );
		$jDebit = filter_input ( INPUT_POST, 'jDebit', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		$jCredit = filter_input ( INPUT_POST, 'jCredit', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		$delJ = (filter_input ( INPUT_POST, 'delJ', FILTER_SANITIZE_NUMBER_INT ) >= 1) ? filter_input ( INPUT_POST, 'delJ', FILTER_SANITIZE_NUMBER_INT ) : 0;

		$dropJ = $db->prepare ( "DELETE FROM $myFLedger WHERE refNumber = ? AND typeCode = '5'" );
		$dropJ->execute ( array (
				$je
		) );
		if ($delJ >= 1) {
			...
			$newId = ($je == 0) ? getNext ( 5, $db, $myFLedger ) : $je;

			if ($jContactNameSelect >= 1) {
				$contactId = $jContactNameSelect;
			} elseif ($jContactName != " " && $jContactName != "") {
				$contactId = conCheck ( $jContactName, $db, $myContacts, $time, '0' );
			} else {
				$contactId = 0;
			}

			$getJAcc = $db->prepare ( "SELECT accountNumber FROM $myFAccounts WHERE id = ?" );
	...
	if (filter_input ( INPUT_POST, 'gTypeCode', FILTER_SANITIZE_NUMBER_INT )) {
		$gTypeCode = filter_input ( INPUT_POST, 'gTypeCode', FILTER_SANITIZE_NUMBER_INT );
		$je = filter_input ( INPUT_POST, 'je', FILTER_SANITIZE_NUMBER_INT );
		$gDate = date2mktime ( filter_input ( INPUT_POST, 'gDate', FILTER_SANITIZE_NUMBER_INT ), 'noon' );
		$gContactName = htmlEntities ( trim ( filter_input ( INPUT_POST, 'gContactName', FILTER_SANITIZE_STRING ) ) );
		$gContactNameSelect = filter_input ( INPUT_POST, 'gContactNameSelect', FILTER_SANITIZE_NUMBER_INT );
		...
		$delG = (filter_input ( INPUT_POST, 'delG', FILTER_SANITIZE_NUMBER_INT ) >= 1) ? filter_input ( INPUT_POST, 'delG', FILTER_SANITIZE_NUMBER_INT ) : 0;

		$dropG = $db->prepare ( "DELETE FROM $myFLedger WHERE refNumber = ? AND typeCode = ?" );
		$dropG->execute ( array (
				$je,
				$gTypeCode
		) );
		if ($delG >= 1) {
			...
			$newId = ($je == 0) ? getNext ( 6, $db, $myFLedger ) : $je;
			$je = 0;

			if ($gContactNameSelect >= 1) {
				$contactId = $gContactNameSelect;
			} elseif ($gContactName != " " && $gContactName != "") {
				$contactId = conCheck ( $gContactName, $db, $myContacts, $time, '0' );
			} else {
				$contactId = 0;
			}
			...
			foreach ( $_POST as $key => $val ) {
				...
				if (preg_match ( "/^gDescription([1-9][0-9]*)$/", $key, $match )) {
					$v = filter_var ( htmlEntities ( trim ( $val ), ENT_QUOTES ), FILTER_SANITIZE_STRING );
					$general [$match [1]] [1] = $v;
				}
...
<div style="float: left;">
	<div
        ...
        <?php
	$getBanks = $db->prepare ( "SELECT id, accountName FROM $myFAccounts WHERE (accountNumber >= ? AND accountNumber <= ?) OR (accountNumber >= ? AND accountNumber <= ?) ORDER BY accountName" );
