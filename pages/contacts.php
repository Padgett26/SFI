...
<?php
if ($myId >= 1) {
    $upId = (filter_input(INPUT_GET, 'contactId', FILTER_SANITIZE_NUMBER_INT)) ? filter_input(
            INPUT_GET, 'contactId', FILTER_SANITIZE_NUMBER_INT) : 0;
    ...
    if (filter_input(INPUT_POST, 'payment', FILTER_SANITIZE_NUMBER_INT) == 1) {
        ...
        $ckNumd = (filter_input(INPUT_POST, 'ckNumd', FILTER_SANITIZE_NUMBER_INT) >=
                1) ? filter_input(INPUT_POST, 'ckNumd',
                FILTER_SANITIZE_NUMBER_INT) : 0;
        $oweAmt = filter_input(INPUT_POST, 'oweAmt',
                FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
        $ccco = filter_input(INPUT_POST, 'ccco', FILTER_SANITIZE_NUMBER_INT);
        $ckNumo = (filter_input(INPUT_POST, 'ckNumo', FILTER_SANITIZE_NUMBER_INT) >=
                1) ? filter_input(INPUT_POST, 'ckNumo',
                FILTER_SANITIZE_NUMBER_INT) : 0;
        $acc = filter_input(INPUT_POST, 'acc', FILTER_SANITIZE_NUMBER_FLOAT,
                    ...
                    if ($rcvId == 0 && $val >= 0.01) {
                        ...
                        if ($bgetR) {
                            ...
                            if ($balancePaid == 0 && $dueAmt >= $startingBalance) {
                                $bset1 = $db->prepare(
                                        "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,?,'0.00',?,?,'0',?,'0')");
                                $bset1->execute(
                                        array(
                                                $time,
                                                $upId,
                                ...
                                $bset3 = $db->prepare(
                                        "UPDATE $myContacts SET balancePaid = ? WHERE id = ?");
                                $bset3->execute(array(
                                        '1',
                                        $upId
                                ));
                        ...
                        $old1 = $db->prepare(
                                "SELECT description, refNumber, typeCode, balanceDue FROM $myFLedger WHERE id = ?");
                        $old1->execute(array(
                                $rcvId
                        ));
                        while ($old1R = $old1->fetch()) {
                            ...
                            if ($b >= 0.01) {
                                ...
                                $old4 = $db->prepare(
                                        "UPDATE $myFLedger SET balanceDue = ? WHERE id = ?");
                                $old4->execute(array(
                                        $z,
                                        $rcvId
                                ));
                                if ($z == 0.00 && $typeCode == 1) {
                                    $setSalePaid = $db->prepare(
                                            "UPDATE $mySales SET paid = ?, ccc = ?, ckNum = ? WHERE id = ?");
                                    $setSalePaid->execute(
                                            array(
                                                    '1',
                                                    $cccd,
                                    ...
                                    $setPurchasingPaid = $db->prepare(
                                            "UPDATE $myPurchasing SET paid = ? WHERE id = ?");
                                    $setPurchasingPaid->execute(
                                            array(
                                                    '1',
                                                    $refNumber
                                            ));
        ...
        if ($oweAmt >= 0.01) {
            if ($acc == '110.0') {
                $getRcv = $db->prepare(
                        "SELECT id, balanceDue, refNumber FROM $myFLedger WHERE contact = ? AND accountNumber = '110.0' AND balanceDue >= '0.01' ORDER BY date");
    ...
    if (filter_input(INPUT_POST, 'contactId', FILTER_SANITIZE_STRING)) {
        ...
        $upChargeTax = filter_input(INPUT_POST, 'chargeTax',
                FILTER_SANITIZE_NUMBER_INT);
        $upVendor = filter_input(INPUT_POST, 'vendor',
                FILTER_SANITIZE_NUMBER_INT);
        ...
        if ($delId == $upId) {
            ...
            if ($upId == '0') {
                $upNew = $db->prepare(
                        "INSERT INTO $myContacts VALUES(NULL,?,?,?,?,?,?,?,?,'0.00','0.00','0','0')");
                $upNew->execute(
                        array(
                                $upName,
                                $upAddress1,
                ...
                $upNewGetId = $db->prepare(
                        "SELECT id FROM $myContacts WHERE lastAccessed = ? ORDER BY id DESC LIMIT 1");
                ...
                $update = $db->prepare(
                        "UPDATE $myContacts SET name = ?, address1 = ?, address2 = ?, phone = ?, email = ?, chargeTax = ?, vendor = ? WHERE id = ?");
                $update->execute(
                        array(
                                $upName,
                                $upAddress1,
    ...
    ?></td>
                ...
                <?php
    $getContacts1 = $db->prepare(
            "SELECT id, name FROM $myContacts WHERE vendor = '1' ORDER BY name");
    ...
    ?>
                ...
                <?php
    $getContacts2 = $db->prepare(
            "SELECT id, name FROM $myContacts WHERE vendor = '0' ORDER BY name");
    ...
    ?>> Vendor
            ...
            <tr>
                ...
                <?php
    $checkCount1 = $db->prepare(
            "SELECT COUNT(*) FROM $myFLedger WHERE contact = ?");
    ...
    $checkCount2 = $db->prepare(
            "SELECT COUNT(*) FROM $myInventory WHERE contactId = ?");
    ...
    $checkCount3 = $db->prepare(
            "SELECT COUNT(*) FROM $myPurchasing WHERE contactId = ?");
    ...
    $checkCount4 = $db->prepare(
            "SELECT COUNT(*) FROM $mySales WHERE contactId = ?");
...
<table>
    ...
    $trans = array();
    $getS = $db->prepare(
            "SELECT id, date, debitAmount, creditAmount, refNumber, typeCode, balanceDue FROM $myFLedger WHERE (accountNumber = ? OR accountNumber = ?) AND contact = ? ORDER BY date");
    $getS->execute(array(
    ...
    foreach ($trans as $k => $v) {
        ...
        switch ($v[4]) {
            case 1:
                echo "<a href='index.php?page=sell&viewId=" . $v[3] .
                        "'>Sales invoice #" . $v[3] . "</a>";
                break;
            case 2:
                echo "<a href='index.php?page=buy&viewId=" . $v[3] .
                        "'>Purchasing invoice #" . $v[3] . "</a>";
                break;
            case 5:
                echo "<a href='index.php?page=reports&r=journal&je=" . $v[3] .
                        "'>Cash Transaction #" . $v[3] . "</a>";
                break;
            case 6:
                echo "<a href='index.php?page=reports&r=general&an=" . $v[3] .
                        "'>General Entry #" . $v[3] . "</a>";
                break;
        }
        ?>
    ...
    <td style="text-align:center;">
    <?php
        echo "<input class='due' type='checkbox' name='ticket" . $k . "' value='" .
                $tAmt . "' onclick='totalIt()' checked>";
        ?>
    </td>
</tr>
...
<tr>
    ...
    <?php
    $acc = $db->prepare(
            "SELECT accountNumber, accountName FROM $myFAccounts WHERE accountNumber >= '100.0' AND accountNumber <= '109.9' ORDER BY accountNumber");
    $acc->execute();
