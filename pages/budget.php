...
<?php
	if (filter_input ( INPUT_POST, 'budgetTermUp', FILTER_SANITIZE_NUMBER_INT ) == '1') {
		$budgetTerm = (filter_input ( INPUT_POST, 'budgetTerm', FILTER_SANITIZE_NUMBER_INT )) ? filter_input ( INPUT_POST, 'budgetTerm', FILTER_SANITIZE_NUMBER_INT ) : 0;
	...
	if (filter_input ( INPUT_POST, 'bUp', FILTER_SANITIZE_NUMBER_INT ) == '1') {
		foreach ( $_POST as $key => $val ) {
			if (preg_match ( "/^budget([0-9][0-9]*)$/", $key, $match )) {
				...
				$budgetUp = $db->prepare ( "UPDATE $myFAccounts SET budget = ? WHERE id = ?" );
	...
	if ($budgetTerm == 0) {
		...
		for($k = 100; $k <= 500; $k = $k + 100) {
			...
			$runningYear = 0.00;
			$runningBudget = 0.00;
			$getA = $db->prepare ( "SELECT id, accountNumber, accountName, budget from $myFAccounts WHERE accountNumber >= ? AND accountNumber <= ? ORDER BY accountNumber" );
			$getA->execute ( array (
					$beginAcc,
					$endAcc
			) );
			while ( $ga = $getA->fetch () ) {
				if ($ga) {
					...
					foreach ( $ranges as $v ) {
						...
						$start = $v [0];
						$end = $v [1];
						$currentLedger = $db->prepare ( "SELECT debitAmount, creditAmount FROM $myFLedger WHERE date >= ? AND date <= ? AND accountNumber = ?" );
						...
						$oldLedger = $db->prepare ( "SELECT debitAmount, creditAmount FROM $myFLedgerOld WHERE date >= ? AND date <= ? AND accountNumber = ?" );
