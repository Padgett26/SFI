...
if ($myId >= 1 && $SA == 0) {
	...
	if (filter_input ( INPUT_GET, 'SAid', FILTER_SANITIZE_STRING )) {
		...
		$newPwd = (filter_input ( INPUT_POST, 'newPwd', FILTER_SANITIZE_NUMBER_INT ) == 1) ? 1 : 0;
		$deny = (filter_input ( INPUT_POST, 'deny', FILTER_SANITIZE_NUMBER_INT ) == 1) ? 1 : 0;
		if ($Sid1 == 'new') {
			...
			if ($ceR) {
				...
				if ($count >= 1) {
					...
					$saUp = $db->prepare ( "INSERT INTO users VALUES(NULL,?,?,?,?,?,?,?,?,?,?)" );
					$saUp->execute ( array (
							$name,
							$email,
							$hidepwd,
			...
			if ($Sid2 >= 1) {
				if ($newPwd == 1 && $pwd != "" && $pwd != " ") {
					$salt = mt_rand ( 100000, 999999 );
					$hidepwd = hash ( 'sha512', ($salt . $pwd), FALSE );
					$setP = $db->prepare ( "UPDATE users SET password = ?, salt = ? WHERE id = ?" );
					$setP->execute ( array (
							$hidepwd,
							$salt,
							$Sid2
				...
				$setSA = $db->prepare ( "UPDATE users SET name = ?, email = ?, denyAccess = ? WHERE id = ?" );
				$setSA->execute ( array (
						$name,
						$email,
						$deny,
	...
	if (filter_input ( INPUT_POST, 'settingsUp', FILTER_SANITIZE_STRING ) == 'company') {
		...
		$settingsUp = $db->prepare ( "UPDATE $mySettings SET name = ?, address1 = ?, address2 = ?, phone = ?, email = ?, currency = ? WHERE id = ?" );
	...
	if (filter_input ( INPUT_POST, 'createBackup', FILTER_SANITIZE_NUMBER_INT ) == 1) {
		...
		while ( $getLR = $getL->fetch () ) {
			...
			$contact = str_replace ( ",", "", html_entity_decode ( getContact ( $getLR ['contact'], $db, $myContacts ), ENT_QUOTES ) );
		...
		if (($cs + $cp) >= 1) {
			...
			if ($cs >= 1) {
				...
				while ( $s2f = $sales2Finalize->fetch () ) {
					...
					echo "<a href='index.php?page=sell&viewId=$s2fId'>" . date ( "Y-m-d", $s2fTime ) . " " . getContact ( $s2fContactId, $db, $myContacts ) . "</a><br>";
				}
			}
			echo "<br><br>";
			if ($cp >= 1) {
				...
				while ( $p2f = $pur2Finalize->fetch () ) {
					...
					echo "<a href='index.php?page=buy&viewId=$p2fId'>" . date ( "Y-m-d", $p2fTime ) . " " . getContact ( $p2fContactId, $db, $myContacts ) . "</a><br>";
				}
			}
			echo "<br><br>";
		} else {
	...
	<tr>
		...
		id="show<?php
			...
			<?php
				...
				while ( $getAR3 = $getA3->fetch () ) {
					$date = $getAR3 ['date'];
					$contact = getContact ( $getAR3 ['contact'], $db, $myContacts );
...
<?php
	...
	<tr>
		...
		<?php
		$getSA = $db->prepare ( "SELECT id, name, email, denyAccess FROM users WHERE subOf = ? ORDER BY name" );
		$getSA->execute ( array (
				$myId
		) );
