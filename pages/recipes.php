...
if ($myId >= 1 && $SA == 0) {
	...
	if (filter_input ( INPUT_POST, 'recipeId', FILTER_SANITIZE_STRING )) {
		...
		$upDescription = filter_var ( htmlEntities ( trim ( $_POST ['description'] ), ENT_QUOTES ), FILTER_SANITIZE_STRING );
		$upInstructions = filter_var ( htmlEntities ( trim ( $_POST ['instructions'] ), ENT_QUOTES ), FILTER_SANITIZE_STRING );
		...
		$upCatName = filter_var ( htmlEntities ( trim ( $_POST ['categoryName'] ), ENT_QUOTES ), FILTER_SANITIZE_STRING );
		$upCatSelect = filter_input ( INPUT_POST, 'categorySelect', FILTER_SANITIZE_NUMBER_INT );
		$upCatSubOf = filter_input ( INPUT_POST, 'subOf', FILTER_SANITIZE_NUMBER_INT );
		$upCreate = filter_input ( INPUT_POST, 'create', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		...
		if ($delId == $upId) {
			...
			$catId = ($upCatSelect == 0) ? catCheck ( $upCatName, $upCatSubOf, $db, $myCategories ) : $upCatSelect;

			$upItems = array ();
			foreach ( $_POST as $key => $val ) {
				if (preg_match ( "/^invQty([0-9][0-9]*)$/", $key, $match )) {
					$a = $match [1];
					$upItems [$a] [0] = filter_var ( $val, FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
				}
				if (preg_match ( "/^invName([0-9][0-9]*)$/", $key, $match )) {
					$b = $match [1];
					$upItems [$b] [1] = filter_var ( htmlEntities ( trim ( $val ), ENT_QUOTES ), FILTER_SANITIZE_STRING );
				}
				if (preg_match ( "/^invNameSelect([0-9][0-9]*)$/", $key, $match )) {
					$c = $match [1];
					$upItems [$c] [2] = filter_var ( $val, FILTER_SANITIZE_NUMBER_INT );
				}
				if (preg_match ( "/^invUOM([0-9][0-9]*)$/", $key, $match )) {
					$c = $match [1];
					$upItems [$c] [3] = filter_var ( $val, FILTER_SANITIZE_NUMBER_INT );
				}
			}
			...
			foreach ( $upItems as $v1 ) {
				if ($v1 [0] > 0) {
					$v1 [1] = ($v1 [2] == 0) ? invCheck ( $v1 [1], $v1 [3], $db, $myInventory, $time ) : $v1 [2];
					for($j = 0; $j < 2; ++ $j) {
						$upItemsString .= $v1 [$j];
						$upItemsString .= ($j != 1) ? "," : ";";
					}
			...
			if ($upId == 0) {
				$upNew = $db->prepare ( "INSERT INTO $myRecipes VALUES(NULL,?,?,?,?,'0.xxx',?,?,?,'0','0','0')" );
				$upNew->execute ( array (
						$upName,
						$upDescription,
						$upInstructions,
				...
				$upNewGetId = $db->prepare ( "SELECT id FROM $myRecipes WHERE ingredients = ? ORDER BY id DESC LIMIT 1" );
				...
				$update = $db->prepare ( "UPDATE $myRecipes SET name = ?, description = ?, instructions = ?, ingredients = ?, categoryId = ?, cost = ?, price = ? WHERE id = ?" );
			...
			if (! empty ( $_FILES ['image'] ['tmp_name'] )) {
				...
				if ($width != null && $height != null) {
					...
					$p1stmt = $db->prepare ( "UPDATE $myRecipes SET picture=? WHERE id=?" );
			...
			if ($upCreate > 0) {
				$getPic = $db->prepare ( "SELECT picture FROM $myRecipes WHERE id = ?" );
				...
				if ($upInvId >= 2) {
					$upDone1 = $db->prepare ( "UPDATE $myInventory SET description = ?, unitOfMeasure = ?, quantity = quantity + ?, cost = ?, price = ?, picture = ?, categoryId = ?, recipeId = ? WHERE id = ?" );
					$upDone1->execute ( array (
							$upDescription,
							$upUOM,
							$upCreate,
							$upCost,
					...
					$upDone3 = $db->prepare ( "INSERT INTO $myInventory VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?,?)" );
					$upDone3->execute ( array (
							$time,
							$upName,
							$upDescription,
							$upUOM,
					...
					$upDone4 = $db->prepare ( "SELECT id FROM $myInventory WHERE name = ? ORDER BY id DESC LIMIT 1" );
				...
				$upDone2 = $db->prepare ( "UPDATE $myRecipes SET invId = ? WHERE id = ?" );
				...
				foreach ( $upItems as $v2 ) {
					if ($v2 [0] > 0) {
						...
						$getQR = $getQ->fetch ();
						$quantity = ($getQR && ($getQR [0] - $sub) >= 0) ? ($getQR [0] - $sub) : 0;
					...
					if ($v2 [1] == 1) {
						$getP = $db->prepare ( "SELECT price FROM $myInventory WHERE id = ?" );
						$getP->execute ( array (
								'1'
						) );
						$getPR = $getP->fetch ();
	...
	?>
                ...
                <?php
	if ($done == 1) {
		echo "<div style='text-align:center; font-weight:bold;'>" . $upCreate . " " . $UOM [$upUOM] . " of " . $upName . " have been added to your inventory, and the ingredients have been adjusted by how much of each you used.</div>";
	} else {
		$name = "";
		$description = "";
		$instructions = "";
		...
		if ($upId >= 1) {
			...
			$getcat = $db->prepare ( "SELECT category FROM $myCategories WHERE id = ?" );
			$getcat->execute ( array (
					$categoryId
			) );
			$getcatR = $getcat->fetch ();
			$categoryName = (! empty ( $getcatR )) ? html_entity_decode ( $getcatR ['category'], ENT_QUOTES ) : "";
		}
		...
		?>" step='.01' placeholder='0.00'><br />
                                    <?php
		echo "<div id='suggestedPrice'>Suggested retail based off of the price amounts in your inventory: " . money ( $suggestedPrice, $currency, $langCode ) . " each.</div>";
		?>
		...
		foreach ( $items as $v ) {
			...
			$getInvName = $db->prepare ( "SELECT name, cost, price, unitOfMeasure, quantity FROM $myInventory WHERE id = ?" );
			...
			if ($ginRow) {
				...
				echo ($i [1] != 1) ? money ( $total, $currency, $langCode ) : money ( $tLabor, $currency, $langCode );
				?></td>
