...
if ($myId >= 1) {
	$msg = "";
	$invId = filter_input ( INPUT_GET, 'grabInvId', FILTER_SANITIZE_NUMBER_INT ) ? filter_input ( INPUT_GET, 'grabInvId', FILTER_SANITIZE_NUMBER_INT ) : 0;
	...
	if (filter_input ( INPUT_POST, 'invId', FILTER_SANITIZE_STRING )) {
		...
		$upDesc = filter_var ( htmlEntities ( trim ( $_POST ['description'] ), ENT_QUOTES ), FILTER_SANITIZE_STRING );
		$upQuantity = filter_input ( INPUT_POST, 'quantity', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		$upUOM = filter_input ( INPUT_POST, 'unitOfMeasure', FILTER_SANITIZE_NUMBER_INT );
		$upCost = filter_input ( INPUT_POST, 'cost', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		$upPrice = filter_input ( INPUT_POST, 'price', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION );
		$upCategoryName = (filter_input ( INPUT_POST, 'categoryName', FILTER_SANITIZE_STRING )) ? filter_var ( htmlEntities ( trim ( $_POST ['categoryName'] ), ENT_QUOTES ), FILTER_SANITIZE_STRING ) : "UNCATEGORIZED";
		$upSubOf = filter_input ( INPUT_POST, 'subOf', FILTER_SANITIZE_NUMBER_INT );
		$upCategorySelect = filter_input ( INPUT_POST, 'categorySelect', FILTER_SANITIZE_NUMBER_INT );
		$upContactName = (filter_input ( INPUT_POST, 'contactName', FILTER_SANITIZE_STRING )) ? filter_var ( htmlEntities ( trim ( $_POST ['contactName'] ), ENT_QUOTES ), FILTER_SANITIZE_STRING ) : "Vendors";
		$upContactSelect = filter_input ( INPUT_POST, 'contactSelect', FILTER_SANITIZE_NUMBER_INT );
		$upTaxed = (filter_input ( INPUT_POST, 'taxed', FILTER_SANITIZE_NUMBER_INT ) == "1") ? "1" : "0";
		$delId = (filter_input ( INPUT_POST, 'delId', FILTER_SANITIZE_NUMBER_INT ) == "1") ? "1" : "0";
		settype ( $upQuantity, "float" );
		settype ( $upCost, "float" );
		settype ( $upPrice, "float" );
		$value = ($upQuantity * $upCost);

		$category = ($upCategorySelect == '0') ? catCheck ( $upCategoryName, $upSubOf, $db, $myCategories ) : $upCategorySelect;

		$contact = ($upContactSelect == '0') ? conCheck ( $upContactName, $db, $myContacts, $time, '1' ) : $upContactSelect;

		if ($delId == "1") {
			$delInv = $db->prepare ( "UPDATE $myInventory SET categoryId = ? WHERE id = ?" );
			...
			if ($invId == 0) {
				$newInv = $db->prepare ( "INSERT INTO $myInventory VALUES(NULL,?,?,?,?,?,?,?,'0.xxx',?,?,'0','0')" );
				$newInv->execute ( array (
						$time,
						$upName,
						$upDesc,
						$upUOM,
				...
				$newInvGetId = $db->prepare ( "SELECT id FROM $myInventory WHERE time = ? && name = ? ORDER BY id DESC LIMIT 1" );
				...
				$updateInv = $db->prepare ( "UPDATE $myInventory SET time = ?, name = ?, description = ?, unitOfMeasure = ?, quantity = ?, cost = ?, price = ?, contactId = ?, categoryId = ?, taxed = ? WHERE id = ?" );
		...
		if (isset ( $_FILES ["image"] ["tmp_name"] ) && $_FILES ["image"] ["size"] >= 1000) {
			...
			if ($width != null && $height != null) {
				...
				$p1stmt = $db->prepare ( "UPDATE $myInventory SET picture=? WHERE id=?" );
	...
	$getRcount = $db->prepare ( "SELECT id FROM $myInventory ORDER BY id DESC LIMIT 1" );
	$getRcount->execute ();
	...
	?></td>
                ...
                <?php
	$getCats = $db->prepare ( "SELECT id, category FROM $myCategories WHERE subOf = ? ORDER BY category" );
	$getCats->execute ( array (
			"0"
	) );
	while ( $gic = $getCats->fetch () ) {
		$catId = $gic ['id'];
		$catCategory = strtoupper ( html_entity_decode ( $gic ['category'], ENT_QUOTES ) );
		?>
		...
		?>;'>
                        <?php
		$getCats2 = $db->prepare ( "SELECT id, category FROM $myCategories WHERE subOf = ? ORDER BY category" );
		$getCats2->execute ( array (
				$catId
		) );
		while ( $gic2 = $getCats2->fetch () ) {
			$catId2 = $gic2 ['id'];
			$catCategory2 = strtoupper ( html_entity_decode ( $gic2 ['category'], ENT_QUOTES ) );
			?>
			...
			?>;'>
                                    <?php
			$getInv1 = $db->prepare ( "SELECT id, name, unitOfMeasure, quantity, recipeId FROM $myInventory WHERE categoryId = ? ORDER BY name" );
		...
		$getInv2 = $db->prepare ( "SELECT id, name, unitOfMeasure, quantity, recipeId FROM $myInventory WHERE categoryId = ? ORDER BY name" );
	...
	?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
	echo ($invId != 1) ? "Suggested retail based off of the " . ($markUp * 100) . "% mark-up in settings: " . money ( (1 + $markUp) * $cost, $currency, $langCode ) . "<br />" : "";
	?>
	...
	if ($invId != 1) {
		?>
                            <tr>
                                ...
                                <td>Select: <select id='categorySelect' name='categorySelect' size='1'>
                                        ...
                                        <?php
		$getCategorySelect = $db->prepare ( "SELECT * FROM $myCategories WHERE subOf = ? ORDER BY category" );
		...
		while ( $gcs = $getCategorySelect->fetch () ) {
			...
			$getCategorySelectSub = $db->prepare ( "SELECT * FROM $myCategories WHERE subOf = ? ORDER BY category" );
		...
		?>")' placeholder="New"> Sub-category of: <select name="subOf" size="1">
                                        ...
                                        <?php
		$newCatSelect = $db->prepare ( "SELECT * FROM $myCategories WHERE subOf = ? ORDER BY category" );
