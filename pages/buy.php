...
if ($myId >= 1 && $SA == 0) {
    ...
    if (filter_input(INPUT_POST, 'sellId', FILTER_SANITIZE_STRING)) {
        ...
        $fromAcc = (filter_input(INPUT_POST, 'fromAcc',
                FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION));
        ...
        if ($delSale == $upId) {
            ...
            foreach ($_POST as $key => $val) {
                ...
                if (preg_match("/^invUOM([0-9][0-9]*)$/", $key, $match)) {
                    $upItems[$match[1]][1] = filter_var($val,
                            FILTER_SANITIZE_NUMBER_INT);
                }
            ...
            foreach ($upItems as $v1) {
                if ($v1[0] >= .01) {
                    ...
                    settype($v1[3], "float");
                    settype($v1[4], "int");
                    $v1[2] = ($v1[4] == 0) ? invCheck($v1[2], $v1[1],
            ...
            if ($upId == 0) {
                $upNew = $db->prepare(
                        "INSERT INTO $myPurchasing VALUES(NULL,?,?,?,?,?,?,'0',?,?,'0')");
                $upNew->execute(
                        array(
                                $upDate,
                                $contactId,
                ...
                $upNewGetId = $db->prepare(
                        "SELECT id FROM $myPurchasing WHERE items = ? ORDER BY id DESC LIMIT 1");
                ...
                $update = $db->prepare(
                        "UPDATE $myPurchasing SET time = ?, contactId = ?, items = ?, taxes = ?, shipping = ?, fees = ?, paid = ?, fromAcc = ? WHERE id=?");
                $update->execute(
                        array(
                                $upDate,
                                $contactId,
            ...
            if ($upDone == 1) {
                ...
                for ($i = 0; $i < $size; ++ $i) {
                    ...
                    $upItems[$i][2] = ($upItems[$i][4] == 0) ? invCheck(
                            $upItems[$i][2], $upItems[$i][1], $myInventory,
                            $time) : $upItems[$i][4];
                }
                ...
                if ($purchasingCostProcessing == 0) {
                    ...
                    for ($i = 0; $i < $uiCount; ++ $i) {
                        if ($upItems[$i][0] >= .01) {
                            $upItems[$i][3] = (($percentItems[$i][0] *
                                    $extraItems + $upItems[$i][3]) /
                                    $upItems[$i][0]);

                            $updateInv = $db->prepare(
                                    "UPDATE $myInventory SET quantity = quantity + ?, cost = ? WHERE id = ?");
                            $updateInv->execute(
                                    array(
                                            $upItems[$i][0],
                                            $upItems[$i][3],
                    ...
                    // typeCode, dailyConfirm, balanceDue, notUsed2
                    if ($tot >= 0.01) {
                ...
                $finalize = $db->prepare(
                        "UPDATE $myPurchasing SET finalized = '1' WHERE id = ?");
