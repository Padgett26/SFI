...
if ($myId >= 1) {
    ...
    if (filter_input(INPUT_POST, 'dateRangeEnd', FILTER_SANITIZE_NUMBER_INT)) {
        $_SESSION['dateRangeEnd'] = date2mktime(
                filter_input(INPUT_POST, 'dateRangeEnd',
                        FILTER_SANITIZE_NUMBER_INT), 'end');
    }
    ...
    if (filter_input(INPUT_POST, 'getSellId', FILTER_SANITIZE_NUMBER_INT)) {
        ...
        $sellId = filter_input(INPUT_POST, 'getSellId',
    ...
    if (filter_input(INPUT_POST, 'sellId', FILTER_SANITIZE_STRING)) {
        ...
        $upNameSelect = filter_input(INPUT_POST, 'contactNameSelect',
                FILTER_SANITIZE_NUMBER_INT);
        $upDate = date2mktime(
                filter_input(INPUT_POST, 'date', FILTER_SANITIZE_NUMBER_INT),
                'noon');
        $upShipping = filter_input(INPUT_POST, 'shipping',
                FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
        $upFees = filter_input(INPUT_POST, 'fees', FILTER_SANITIZE_NUMBER_FLOAT,
                FILTER_FLAG_ALLOW_FRACTION);
        $upDiscountAmount = filter_input(INPUT_POST, 'discountAmount',
                FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
        $upDone = (filter_input(INPUT_POST, 'done', FILTER_SANITIZE_NUMBER_INT) ==
                1) ? 1 : 0;
        $taxExempt = (filter_input(INPUT_POST, 'taxExempt',
                FILTER_SANITIZE_STRING) == "Y") ? 0 : 1;
        settype($taxExempt, "int");
        $upTaxes = (filter_input(INPUT_POST, 'taxes',
                FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION) *
                $taxExempt);
        ...
        $paid = (filter_input(INPUT_POST, 'paid', FILTER_SANITIZE_NUMBER_INT) >=
                1) ? 1 : 0;
        $ccc = (filter_input(INPUT_POST, 'paid', FILTER_SANITIZE_NUMBER_INT) >= 1) ? filter_input(
                INPUT_POST, 'paid', FILTER_SANITIZE_NUMBER_INT) : 0;
        $ckNum = (filter_input(INPUT_POST, 'ckNum', FILTER_SANITIZE_NUMBER_INT));
        $notes = filter_var(htmlEntities(trim($_POST['notes']), ENT_QUOTES),
                FILTER_SANITIZE_STRING);
        $upDiscountPercent = filter_input(INPUT_POST, 'discountPercent',
                FILTER_SANITIZE_NUMBER_INT);
        $delSale = (filter_input(INPUT_POST, 'delSale',
                FILTER_SANITIZE_NUMBER_INT) == 1) ? $upId : "N";
        ...
        if ($delSale == $upId) {
            ...
            $contactId = ($upNameSelect == 0) ? conCheck($upName, $myContacts,
                    $time, '0') : $upNameSelect;

            $updateTax = $db->prepare(
                    "UPDATE $myContacts SET chargeTax = ? WHERE id = ?");
            ...
            foreach ($upItems as $v1) {
                ...
                settype($v1[5], "int");
                if ($v1[0] >= 0.01) {
                    $v1[1] = ($v1[4] == 0) ? invCheck($v1[1], $v1[5],
                            $myInventory, $time) : $v1[4];
                ...
                if ($cp) {
                    ...
                    if ($p == 0.00) {
                        $setPrice = $db->prepare(
                                "UPDATE $myInventory SET price = ? WHERE id = ?");
            ...
            if ($upId == 0) {
                $upNew = $db->prepare(
                        "INSERT INTO $mySales VALUES(NULL,?,?,?,?,?,?,'0',?,?,?,?,?,'0')");
                $upNew->execute(
                        array(
                                $upDate,
                                $contactId,
                ...
                $upNewGetId = $db->prepare(
                        "SELECT id FROM $mySales WHERE items = ? ORDER BY id DESC LIMIT 1");
                ...
                $update = $db->prepare(
                        "UPDATE $mySales SET time = ?, contactId = ?, items = ?, taxes = ?, shipping = ?, fees = ?, paid = ?, discountPercent = ?, ccc = ?, ckNum = ?, notes = ? WHERE id=?");
                $update->execute(
                        array(
                                $upDate,
                                $contactId,
            ...
            if ($upDone == 1) {
                ...
                foreach ($upItems as $v2) {
                    ...
                    settype($v2[4], "int");
                    settype($v2[5], "int");
                    $v2[1] = ($v2[4] == 0) ? invCheck($v2[1], $v2[5],
                            $myInventory, $time) : $v2[4];
                    if ($v2[0] >= .01) {
                        $getQ = $db->prepare(
                        ...
                        $getQR = $getQ->fetch();
                        if ($getQR) {
                            $quantity = (($getQR['quantity'] - $v2[0]) >= 0) ? ($getQR['quantity'] -
                                    $v2[0]) : 0;
                        } else {
                            $quantity = 0;
                        }
                ...
                $tot = ($totalPriceLedger + $upShipping + $upFees + $upTaxes -
                        $upDiscountAmount);
                ...
                if ($tot >= 0.01) {
                    $upLedgerTotal = $db->prepare(
                            "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,?,'0.00',?,?,'0',?,'0')");
                    $upLedgerTotal->execute(
                            array(
                                    $upDate,
                                    $contactId,
                                    "Sales Receipt",
                ...
                if ($totalPriceLedger >= 0.01) {
                    $upLedgerPrice = $db->prepare(
                            "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,'0.00',?,?,?,'0','0','0')");
                    $upLedgerPrice->execute(
                            array(
                                    $upDate,
                                    $contactId,
                                    "Sales Receipt",
                ...
                if ($upShipping >= 0.01) {
                    $upLedgerShipping = $db->prepare(
                            "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,'0.00',?,?,?,'0','0','0')");
                    $upLedgerShipping->execute(
                            array(
                                    $upDate,
                                    $contactId,
                                    "Sales Receipt",
                ...
                if ($upFees >= 0.01) {
                    $upLedgerFees = $db->prepare(
                            "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,'0.00',?,?,?,'0','0','0')");
                    $upLedgerFees->execute(
                            array(
                                    $upDate,
                                    $contactId,
                                    "Sales Receipt",
                ...
                if ($upDiscountAmount >= 0.01) {
                    $upLedgerDiscount = $db->prepare(
                            "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,?,'0.00',?,?,'0','0','0')");
                    $upLedgerDiscount->execute(
                            array(
                                    $upDate,
                                    $contactId,
                                    "Sales Receipt",
                ...
                if ($totalCostLedger >= 0.01) {
                    $upLedgerCostInv = $db->prepare(
                            "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,'0.00',?,?,?,'0','0','0')");
                    $upLedgerCostInv->execute(
                            array(
                                    $upDate,
                                    $contactId,
                                    "Sales Receipt",
                    ...
                    $upLedgerCostCOS = $db->prepare(
                            "INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,?,'0.00',?,?,'0','0','0')");
                    $upLedgerCostCOS->execute(
                            array(
                                    $upDate,
                                    $contactId,
                                    "Sales Receipt",
                ...
                $finalize = $db->prepare(
                        "UPDATE $mySales SET finalized = '1' WHERE id = ?");
