...
if ($sellId >= 1) {
    ...
    $gIR = $getInv->fetch();
    $SDate = ($gIR['time'] >= 1) ? date("Y-m-d", $gIR['time']) : date("Y-m-d",
            $time);
    ...
    $getCName = $db->prepare(
            "SELECT name, chargeTax FROM $myContacts WHERE id = ?");
    ...
    if ($items2 != "") {
        foreach ($items2 as $var) {
            ...
            $getInvName = $db->prepare(
                    "SELECT price,taxed FROM $myInventory  WHERE id = ?");
            ...
            if ($lineInvId != 1 && $SChargeTax == 1 && $lineTaxed == 1) {
                $tax += ((($lineQty * $linePrice) -
                        (($lineQty * $linePrice) * ($SDiscountPercent / 100))) *
                        $taxRate);
            }
...
<form action="index.php?page=sell" method="post">
    ...
    if ($items2 != "") {
        foreach ($items2 as $var) {
            ...
            $getInvName = $db->prepare(
                    "SELECT name, cost, price, unitOfMeasure, taxed FROM $myInventory WHERE id = ?");
            $getInvName->execute(array(
                    $lineInvId
            ));
            $ginRow = $getInvName->fetch();
            ...
            if ($linePrice >= .01) {
                echo money_sfi($lineQty * $linePrice, $currency, $langCode) .
                        "<input type='hidden' name='invPrice$j' value='$linePrice'>";
            } else {
    ...
    echo "<td style='padding:5px; border:1px solid #000000; text-align:center;'>" .
            money_sfi($tax, $currency, $langCode) .
            "<input type='hidden' name='taxes' value='$tax'><br />Tax Exempt? <input type='checkbox' name='taxExempt' value='Y'";
    ...
    echo "</select></td>\n";
    echo "<td style='padding:5px; border:1px solid #000000; text-align:center;'>(" .
            money_sfi($discount, $currency, $langCode) .
            ")<input type='hidden' name='discountAmount' value='$discount'></td>\n";
    echo "</tr>\n";
    echo "<tr>\n";
    echo "<td></td>\n";
    ...
    echo "<td style='padding:5px; border:1px solid #000000; text-align:center;'>" .
            money_sfi($totalBill, $currency, $langCode) . "</td>\n";
    echo "</tr>\n";
    echo "<tr>\n";
