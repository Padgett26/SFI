...
<?php
$getBal = $db->prepare ( "SELECT startBalance FROM $myFAccounts WHERE accountNumber = ?" );
...
$newBal = $db->prepare ( "SELECT debitAmount, creditAmount FROM $myFLedger WHERE accountNumber = '101.0' && dailyConfirm = '1' && date >= ? ORDER BY date" );
...
<?php
$totCredit = 0;
$getPur1 = $db->prepare ( "SELECT creditAmount FROM $myFLedger WHERE accountNumber = '101.0' && creditAmount >= '0.01' && dailyConfirm = '0' ORDER BY date" );
$getPur1->execute ();
...
<?php
$confirm = array ();
$getPur = $db->prepare ( "SELECT * FROM $myFLedger WHERE accountNumber = '101.0' && creditAmount >= '0.01' && dailyConfirm = '0' ORDER BY date" );
$getPur->execute ();
while ( $getPurR = $getPur->fetch () ) {
	$pId = $getPurR ['id'];
	$pDate = $getPurR ['date'];
	$pContact = html_entity_decode ( getContact ( $getPurR ['contact'], $db, $myContacts ), ENT_QUOTES );
...
$totDebit = 0;
$getSale1 = $db->prepare ( "SELECT debitAmount FROM $myFLedger WHERE accountNumber = '101.0' && debitAmount >= '0.01' && dailyConfirm = '0' ORDER BY date" );
$getSale1->execute ();
...
$check = 0;
$cc = 0;
$getPur = $db->prepare ( "SELECT * FROM $myFLedger WHERE accountNumber = '101.0' && debitAmount >= '0.01' && dailyConfirm = '0' ORDER BY date" );
$getPur->execute ();
while ( $getPurR = $getPur->fetch () ) {
	$pId = $getPurR ['id'];
	$pDate = $getPurR ['date'];
...
<?php
$getA = $db->prepare ( "SELECT id, accountName FROM $myFAccounts WHERE accountNumber >= '100.0' && accountNumber <= '101.9' AND accountNumber != '101.0' ORDER BY accountNumber" );
$getA->execute ();
