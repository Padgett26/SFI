...
$getCK = $db->prepare(
        "SELECT cashCheckCC, checkNumber FROM $myFLedger WHERE refNumber = ? AND typeCode = ? LIMIT 1");
$getCK->execute(array(
        $sellId,
        '2'
));
...
<form action="index.php?page=buy" method="post">
    ...
    <table style="border: 1px solid #000000;">
        <?php
        echo "<tr>\n";
        echo "<td style='padding:5px; border:1px solid #000000; text-align:center;'><input type='text' name='contactName' value=\"" .
                $SContactName .
        ...
        echo "<td style='padding:5px; border:1px solid #000000; text-align:center;'>" .
                money_sfi($total + $SShipping + $SFees + $STaxes, $currency,
                        $langCode) . "</td>\n";
        echo "</tr>\n";
        echo "<tr>\n";
        ...
        $banks = $db->prepare(
                "SELECT accountNumber, accountName FROM $myFAccounts WHERE (accountNumber >= ? && accountNumber <= ?) || (accountNumber >= ? && accountNumber <= ?) ORDER BY accountNumber");
        $banks->execute(array(
                '101.0',
                '101.9',
                '210.1',
