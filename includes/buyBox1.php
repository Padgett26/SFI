...
<div style="">
    ...
    if ($viewId == 0) {
        $transactions = $db->prepare(
                "SELECT * FROM $myPurchasing WHERE time >= ? && time <= ? ORDER BY time");
    ...
    while ($transRow = $transactions->fetch()) {
        ...
        $cName = html_entity_decode(getContact($transContact, $myContacts),
                ENT_QUOTES);
        ...
        $transactions2 = $db->prepare(
                "SELECT cashCheckCC, checkNumber FROM $myFLedger WHERE refNumber = ? && typeCode = ? ORDER BY id DESC LIMIT 1");
        ...
        echo "<tr id='showTrans" . $transId .
                "' style='display:block; border:1px solid #000000; $shaded'>\n";
        echo "<td style='width:100px; padding:5px; border:1px solid #000000; text-align:center;'>" .
                date("m / d / Y", $transTime) . "</td>\n";
        ...
        echo "<div id='showPaid" . $transId .
                "' style='position:absolute; right:5px; bottom:5px; font-size:.5em; font-weight:normal; display:block;'>";
        echo ($paid == 1) ? "Paid" : "";
        echo "</div></td>\n";
        echo "<td style='width:50px; padding:5px; border:1px solid #000000;'>" .
                money_sfi($showTotal, $currency, $langCode) . "</td>\n";
        ...
        echo "</tr>\n";
        echo ($transId == $viewId) ? "<tr id='editTrans" . $transId .
                "' style='display:block; border:1px solid #000000;'>\n" : "<tr id='editTrans" .
                $transId . "' style='display:none; border:1px solid #000000;'>\n";
        ...
        foreach ($items as $val) {
            ...
            $getInvName = $db->prepare(
                    "SELECT name FROM $myInventory WHERE id = ?");
            ...
            echo "<td style='padding:5px; border:1px solid #000000; text-align:center;'>" .
                    $UOM[$lineUOM] . "</td>\n";
            echo "<td style='padding:5px; border:1px solid #000000; text-align:center;'>" .
                    money_sfi($lineCost, $currency, $langCode) . "</td>\n";
            echo "</tr>\n";
        }
        echo "<tr>\n";
        ...
        echo "<td style='padding:5px; border:1px solid #000000; text-align:center;'>" .
                money_sfi($total, $currency, $langCode) . "</td>\n";
        echo "</tr>\n";
        echo "</table></td>\n";
        echo "</tr>\n";
        $shade ++;
