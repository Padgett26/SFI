...
if ($je == 0) {
    ...
    $getG1 = $db->prepare(
            "SELECT date, contact, cashCheckCC, checkNumber FROM $myFLedger WHERE refNumber = ? AND typeCode = ? LIMIT 1");
    $getG1->execute(array(
            $je,
            $type
    ));
...
if ($je >= 1) {
    $getG2 = $db->prepare(
            "SELECT description, accountNumber, debitAmount, creditAmount FROM $myFLedger WHERE refNumber = ? AND typeCode = ?");
    $getG2->execute(array(
            $je,
            $type
    ));
...
<td><input id="D<?php
        ...
        ?>" value="<?php
        echo $gDebit;
        ?>" step="0.01" onchange="generalBalanceCheck()"></td>
<td><input id="C<?php
        echo $i;
...
<td><input id="D<?php
    ...
    ?>" type="number" name="gDebit<?php
    echo $i;
    ?>" value="0.00" step="0.01" onchange="generalBalanceCheck()"></td>
<td><input id="C<?php
    echo $i;
    ?>" type="number" name="gCredit<?php
    echo $i;
