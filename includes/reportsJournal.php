...
<table class='table1'>
    ...
    <tr>
        ...
        <td><input type="date" name="jDate" value="<?php
								...
								echo $dateMin;
								?>'></td>
        <td style=''><input type='text' name='jContactName' value='' placeholder='contact' oninput='getContactSelect("contactSelect", this.value, <?php
								echo $myId;
								?>)'><br>
												...
												while ($getLR = $getL->fetch()) {
													...
													if ($date >= $dateRangeStart && $date <= $dateRangeEnd) {
														...
														while ($getJR = $getJ->fetch()) {
															$xId = $getJR['id'];
															$xdate = $getJR['date'];
															$xcontact = html_entity_decode(getContact($getJR['contact'], $myContacts), ENT_QUOTES);
