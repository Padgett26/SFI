...
<table class='table1'>
    ...
    <tr>
        ...
        echo (isset($_SESSION['jDate'])) ? date("Y-m-d", $_SESSION['jDate']) : date(
                "Y-m-d", $time);
        ?>" min='<?php
        echo $dateMin;
        ?>'></td>
            ...
            while ($getLR = $getL->fetch()) {
                ...
                if ($date >= $dateRangeStart && $date <= $dateRangeEnd && $t == 0) {
                    ...
                    echo "<td style='text-align:right;'>" .
                            money_sfi($accStartBalance, $currency, $langCode) .
                            "</td>\n";
                ...
                if ($date >= $dateRangeStart && $date <= $dateRangeEnd) {
                    $getJ = $db->prepare(
                            "SELECT * FROM $myFLedger WHERE refNumber = ? AND typeCode = ? AND id != ?");
                    $getJ->execute(array(
                            $refNumber,
                            $type,
                            $id
                    ...
                    while ($getJR = $getJ->fetch()) {
                        $xId = $getJR['id'];
                        $xdate = $getJR['date'];
                        $xcontact = html_entity_decode(
                                getContact($getJR['contact'], $myContacts),
                                ENT_QUOTES);
                        $xdescription = html_entity_decode(
                                $getJR['description'], ENT_QUOTES);
                        ...
                        echo "<td>" . $xaccountNumber . " - " .
                                $ACCOUNTS[$xaccountNumber] . "</td>\n";
                        echo "<td style='text-align:right;'>";
                        echo ($xcreditAmount > 0.00) ? money_sfi($xcreditAmount,
                                $currency, $langCode) : "";
                        echo "</td>\n";
                        echo "<td style='text-align:right;'>";
                        echo ($xdebitAmount > 0.00) ? money_sfi($xdebitAmount,
                                $currency, $langCode) : "";
                        echo "</td>\n";
                        echo "<td style='text-align:right;'>" .
                                money_sfi($accStartBalance, $currency, $langCode) .
