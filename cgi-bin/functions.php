...
function createSettings($table, $db) {
	$settings = $db->prepare ( "CREATE TABLE $table (
...
budgetTerm INT(2) DEFAULT 0
	...
	$settings->execute ();
	$settings2 = $db->prepare ( "INSERT INTO $table VALUES(1,'name','address','city st zip','phone','email','0','0.0000','0','0','USD','0')" );
	$settings2->execute ();
	return true;
}
function createCategories($table, $db) {
	$categories = $db->prepare ( "CREATE TABLE $table (
...
notUsed2 INT(2) DEFAULT 0
	...
	$categories->execute ();
	$categories1 = $db->prepare ( "INSERT INTO $table VALUES(1,'Labor','0','0','0')" );
	$categories1->execute ();
	$categories2 = $db->prepare ( "INSERT INTO $table VALUES(2,'Uncategorized','0','0','0')" );
	$categories2->execute ();
	$categories3 = $db->prepare ( "INSERT INTO $table VALUES(3,'Hidden','0','0','0')" );
	$categories3->execute ();
	return true;
}
...
notUsed2 INT(2) DEFAULT 0
	...
	$acc2 = $db->prepare ( "INSERT INTO $table VALUES('2','120.0','asset','Inventory','0.00','0.00',?,'0.00','0','0')" );
	$acc2->execute ( array (
			$time
	) );
	$acc3 = $db->prepare ( "INSERT INTO $table VALUES('3','200.0','liability','Sales Tax Payable','0.00','0.00',?,'0.00','0','0')" );
	$acc3->execute ( array (
			$time
	) );
	$acc4 = $db->prepare ( "INSERT INTO $table VALUES('4','300.0','capital','Owners Capital','0.00','0.00',?,'0.00','0','0')" );
	$acc4->execute ( array (
			$time
	) );
	$acc5 = $db->prepare ( "INSERT INTO $table VALUES('5','400.1','income','Revenue','0.00','0.00',?,'0.00','0','0')" );
	$acc5->execute ( array (
			$time
	) );
	$acc6 = $db->prepare ( "INSERT INTO $table VALUES('6','400.2','income','Shipping COS','0.00','0.00',?,'0.00','0','0')" );
	$acc6->execute ( array (
			$time
	) );
	$acc7 = $db->prepare ( "INSERT INTO $table VALUES('7','500.0','expense','Sales Tax Expense','0.00','0.00',?,'0.00','0','0')" );
	$acc7->execute ( array (
			$time
	) );
	$acc8 = $db->prepare ( "INSERT INTO $table VALUES('8','220.0','liability','Labor Production','0.00','0.00',?,'0.00','0','0')" );
	$acc8->execute ( array (
			$time
	) );
	$acc9 = $db->prepare ( "INSERT INTO $table VALUES('9','400.3','income','Inventory Adj','0.00','0.00',?,'0.00','0','0')" );
	$acc9->execute ( array (
			$time
	) );
	$acc10 = $db->prepare ( "INSERT INTO $table VALUES('10','110.0','asset','Account Receivables','0.00','0.00',?,'0.00','0','0')" );
	$acc10->execute ( array (
			$time
	) );
	$acc11 = $db->prepare ( "INSERT INTO $table VALUES('11','400.4','income','Fees Revenue','0.00','0.00',?,'0.00','0','0')" );
	$acc11->execute ( array (
			$time
	) );
	$acc12 = $db->prepare ( "INSERT INTO $table VALUES('12','400.5','income','Discounts','0.00','0.00',?,'0.00','0','0')" );
	$acc12->execute ( array (
			$time
	) );
	$acc13 = $db->prepare ( "INSERT INTO $table VALUES('13','400.6','income','Inventory COS','0.00','0.00',?,'0.00','0','0')" );
	$acc13->execute ( array (
			$time
	) );
	$acc14 = $db->prepare ( "INSERT INTO $table VALUES('14','380.0','capital','Net Income','0.00','0.00',?,'0.00','0','0')" );
	$acc14->execute ( array (
			$time
	) );
	$acc15 = $db->prepare ( "INSERT INTO $table VALUES('15','101.1','asset','Petty Cash','0.00','0.00',?,'0.00','0','0')" );
	$acc15->execute ( array (
			$time
	) );
	$acc16 = $db->prepare ( "INSERT INTO $table VALUES('16','210.0','liability','Accounts Payable','0.00','0.00',?,'0.00','0','0')" );
	$acc16->execute ( array (
			$time
	) );
	$acc17 = $db->prepare ( "INSERT INTO $table VALUES('17','399.9','capital','Retained Earnings','0.00','0.00',?,'0.00','0','0')" );
	$acc17->execute ( array (
			$time
	) );
	return true;
...
function createFLedger($table, $db) {
	$fLedger = $db->prepare ( "CREATE TABLE $table (
id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
function createInventory($table, $db) {
	$inventory = $db->prepare ( "CREATE TABLE $table (
id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
taxed INT(2) UNSIGNED DEFAULT 1
	...
	$inventory->execute ();
	$inventory2 = $db->prepare ( "INSERT INTO $table VALUES(1,?,'Labor','Labor cost per hour','3','0','0.00','0.00','','0','1','0','0')" );
...
function createSales($table, $db) {
	$sales = $db->prepare ( "CREATE TABLE $table (
id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
time INT(12) UNSIGNED,
...
function createPurchasing($table, $db) {
	$purchasing = $db->prepare ( "CREATE TABLE $table (
id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
time INT(12) UNSIGNED,
contactId INT(8) UNSIGNED DEFAULT 0,
items VARCHAR(2000),
...
function createContacts($table, $db) {
	$contacts = $db->prepare ( "CREATE TABLE $table (
id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
name VARCHAR(100),
address1 VARCHAR(100),
address2 VARCHAR(100),
...
prevBalancePaid INT(2) UNSIGNED DEFAULT 0
	...
	$contacts->execute ();
	$contacts1 = $db->prepare ( "INSERT INTO $table VALUES(1,'In House','','','','',?,'1','0','0.00','0','0')" );
...
function createRecipes($table, $db) {
	$recipes = $db->prepare ( "CREATE TABLE $table (
id INT(10) UNSIGNED AUTO_INCREMENT PRIMARY KEY,
name VARCHAR(100),
description TEXT,
instructions TEXT,
...
function money($amt, $symbol, $code) {
	settype ( $amt, "float" );
