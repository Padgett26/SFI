...
if (filter_input(INPUT_POST, 'login', FILTER_SANITIZE_NUMBER_INT) == "1") {
	...
	$login2stmt = $db->prepare("SELECT id FROM users WHERE email = ? AND password = ? && accessLevel >= ?");
	...
	if ($login2row) {
		$x = $login2row['id'];
		$_SESSION['myId'] = $x;
		setcookie("staySignedIn", $_SESSION['myId'], $time + 1209600, "/", $domain, 0); // set
		                                                                                // for
		                                                                                // 14
		                                                                                // days
...
if ($myId >= 1) {
	if (filter_input(INPUT_POST, 'settingsUp', FILTER_SANITIZE_STRING) == 'company') {
		$timeZoneArea = filter_input(INPUT_POST, 'timeZoneArea', FILTER_SANITIZE_STRING);
		$timeZoneCity = filter_input(INPUT_POST, 'timeZoneCity', FILTER_SANITIZE_STRING);
		$timeZone = $timeZoneArea . "/" . $timeZoneCity;

		$settingsUp = $db->prepare("UPDATE $mySettings SET timeZone = ? WHERE id = ?");
	...
	if ($gsRow) {
		...
		$companyEIN = $gsRow['EIN'];
		$companyStateEIN = $gsRow['stateEIN'];
		$overtimeMultiplier = $gsRow['overtimeMultiplier'];
		$holidayMultiplier = $gsRow['holidayMultiplier'];
		$overtimeHours = $gsRow['overtimeHours'];
		$startOfWorkWeek = $gsRow['startOfWorkWeek'];
		$SSRate = $gsRow['SSRate'];
		$SSBaseRate = $gsRow['SSBaseRate'];
		$medicareRate = $gsRow['medicareRate'];
		$medicareBaseRate = $gsRow['medicareBaseRate'];
		$medicareAddOn = $gsRow['medicareAddOn'];
		$futaRate = $gsRow['futaRate'];
		$futaBaseRate = $gsRow['futaBaseRate'];
		$getlangCode = $db->prepare("SELECT langCode FROM currencies WHERE symbol = ?");
...
if (filter_input(INPUT_POST, 'sendFeedback', FILTER_SANITIZE_NUMBER_INT) == 1) {
	$fromEmail = (filter_input(INPUT_POST, 'fromEmail', FILTER_VALIDATE_EMAIL)) ? filter_input(INPUT_POST, 'fromEmail', FILTER_SANITIZE_EMAIL) : 1;
...
$grabInvId = filter_input(INPUT_GET, 'grabInvId', FILTER_SANITIZE_NUMBER_INT) ? filter_input(INPUT_GET, 'grabInvId', FILTER_SANITIZE_NUMBER_INT) : 0;
...
if ($myId >= 1) {
	...
	while ($contactsR = $getContacts->fetch()) {
		$CONTACTS[$contactsR['id']] = html_entity_decode($contactsR['name'], ENT_QUOTES);
	}
...
$ACCOUNTS = array();
if ($myId >= 1) {
	$getAcc = $db->prepare("SELECT accountNumber, accountName FROM $myFAccounts ORDER BY accountNumber");
	$getAcc->execute();
	while ($accR = $getAcc->fetch()) {
		$ACCOUNTS[$accR['accountNumber']] = html_entity_decode($accR['accountName'], ENT_QUOTES);
	}
}
...
if ($myId >= 1) {
	...
	while ($getcatR = $getcat->fetch()) {
		$CATEGORIES[$getcatR['id']] = html_entity_decode($getcatR['category'], ENT_QUOTES);
	}
}
...
$tables = array(
		...
		array(
				...
				'function' => 'createEmployeePayrollHistory'
		)
);
...
if (filter_input(INPUT_POST, 'undoClose', FILTER_SANITIZE_NUMBER_INT) == 1) {
	...
	$cOld = $db->prepare("SELECT * FROM $myFLedgerOld WHERE date >= ? AND date <= ?");
	$cOld->execute(array(
			$ucStart,
			$ucEnd
	));
	while ($coR = $cOld->fetch()) {
		...
		$setOld = $db->prepare("INSERT INTO $myFLedger VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$setOld->execute(array(
				$oDate,
				$oContact,
				$oDescription,
				$oCashCheckCC,
	...
	$getCBal = $db->prepare("SELECT id, prevStartingBalance, prevBalancePaid FROM $myContacts");
	$getCBal->execute();
	while ($gcb = $getCBal->fetch()) {
		...
		$setCBal = $db->prepare("UPDATE $myContacts SET startingBalance = ?, balancePaid = ?, prevStartingBalance = ?, prevBalancePaid = ? WHERE id = ?");
	...
	while ($gab = $getABal->fetch()) {
		...
		$setABal = $db->prepare("UPDATE $myFAccounts SET startBalance = ? WHERE id = ?");
...
if (filter_input(INPUT_POST, 'close', FILTER_SANITIZE_NUMBER_INT) == 1) {
	...
	foreach ($CONTACTS as $k => $v) {
		...
		$setPrevS = $db->prepare("UPDATE $myContacts SET prevStartingBalance = ?, prevBalancePaid = ? WHERE id = ?");
		...
		// Accounts Payable
		$getPay = $db->prepare("SELECT debitAmount, creditAmount FROM $myFLedger WHERE contact = ? AND accountNumber = ? AND date < ?");
		...
		$getR = $db->prepare("SELECT debitAmount, creditAmount FROM $myFLedger WHERE contact = ? AND accountNumber = ? AND date < ?");
		$getR->execute(array(
				$k,
				'110.0',
				$fy
		...
		$setS = $db->prepare("UPDATE $myContacts SET startingBalance = ?, balancePaid = ? WHERE id = ?");
		$setS->execute(array(
				$sb,
				'0',
				$k
	...
	$toClear = array();
	$showFileFY = ($fy - 1);
	$fileName = "fiscalYearBackup" . date("Y-m-d", $fiscalYear) . "to" . date("Y-m-d", $showFileFY) . ".csv";
	...
	while ($getLR = $getL->fetch()) {
		...
		$backup = $db->prepare("INSERT INTO $myFLedgerOld VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?,?,?)");
		$backup->execute(array(
				$getLR['date'],
				$getLR['contact'],
				$getLR['description'],
				$getLR['cashCheckCC'],
		...
		$getR = $get->fetch();
		if ($getR) {
			$contact = str_replace(",", "", html_entity_decode($getR['name'], ENT_QUOTES));
		}

		$txt = "$id,$date,$contact,$description,$cashCheckCC,$checkNumber,$accountNumber - $ACCOUNTS[$accountNumber]," . $debitAmount . "," . $creditAmount . ",$refNumber,$typeCode,$reconcile\n";
		fwrite($myfile, $txt);
	}
	$txt = ",,,,,,,,,,,\n";
	fwrite($myfile, $txt);
	...
	$getL2 = $db->prepare("SELECT accountNumber,debitAmount,creditAmount FROM $myFLedger WHERE date < ?");
	$getL2->execute(array(
			$fy
	));
	...
	$updateFY = $db->prepare("UPDATE $mySettings SET fiscalYear = ? WHERE id = '1'");
...
if ($myId >= 1) {
	if ($fiscalYear >= 1) {
		...
		$time2Close = mktime(0, 0, 0, $fycheck[1], $fycheck[2], ($fycheck[0] + 1));
